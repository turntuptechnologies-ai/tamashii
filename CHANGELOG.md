...
## [v0.106.0] — 2026-04-20 — Summer Fireworks Festival (Hanabi 花火)

### Added
- **Summer Fireworks Festival** — the long-awaited **summer-night signature event**, completing the four-season seasonal-night vocabulary that has been building across the last year of cycles. A rare summer-night hanabi (花火 — "fire flower") show where 5 rockets rise from the ground one-by-one and bloom into colored radial starbursts in the upper air. Players *cheer* for each bloom before it fades, and cheering every firework triggers a grand finale blessing. Fills NEXT.md's #1 recurring request for "the last remaining seasonal-night gap" — autumn/winter/spring all had their signature moon events, only summer was silent.
- **Four-stage arc** mirroring the proven seasonal-moon template: (1) **rise** — rockets whistle upward with a trailing sparkle; (2) **bloom** — each rocket erupts into a colored N-point starburst at its apex; (3) **cheer** — player clicks each bloom within its ~4s visible window to "tamaya!" cheer for it; (4) **blessing** — cheering every firework triggers a rainbow finale with happiness/care/XP reward. Same 4-stage cadence as cherry moon (rise → cranes → wish → bless), snow moon (rise → snowflakes → catch → bless), and harvest moon (rise → lanterns → light → bless) — now with summer-night vocabulary layered on top.
- **New verb: *cheer*** — the fourth distinct seasonal-night interaction verb. Autumn *lights* lanterns, winter *catches* snowflakes, spring *wishes* on cranes, and now summer *cheers* for fireworks. The cry is "tamaya!" (玉屋) — the historical Japanese cheer for an exceptionally beautiful hanabi. Distinct emotional register from the other three: loud & celebratory rather than quiet & contemplative.
- **5-color hanabi palette** — crimson (hue 0°), warm gold (38°), vivid emerald (130°), aqua blue (200°), violet (290°). Classic Japanese summer-firework hues. Distinct from the cherry moon's 4 pastel pinks/peaches and the snow moon's single silver-blue. Each show shuffles the hue order so colors don't repeat in the same sequence across sessions.
- **Staggered rocket launches** — one rocket rises every ~3 seconds, so the show unfolds over ~15s of rising/blooming rather than all at once. Each rocket traces a different color, a different starburst count, and a different horizontal position.
- **Per-bloom starburst variety** — spoke counts alternate 5/6/8 across the 5 fireworks so each bloom has a visibly different silhouette. 5-point stars read as classic peonies; 6-point as chrysanthemums; 8-point as big radial sunbursts.
- **Rocket rise sound** — a rising sine whistle sweep from 180 Hz → 1400 Hz over ~0.5s plus a high-pass-filtered sparkle hiss (3500 Hz) trailing behind. The unmistakable *shhhhwip* of a climbing rocket.
- **Bloom BWOOM sound** — three-layer explosion: (1) a low-pass-filtered (900 Hz) noise burst for the boom body, (2) a sub-bass 80→45 Hz sine thump for the chest-hit, and (3) a bandpass-filtered (6500 Hz) pink-noise crackle tail for the characteristic after-sparkle. A real firework sounds, not a generic pop.
- **Per-bloom cheer chime** — warm C major triad (C6 + E6 + G6, sine + triangle) plays on each cheer, detuned by hue index so each firework has a slightly different pitch. Across the 5 hues, the cheers span ~6 semitones — the show ends up playing its own melody.
- **Blessing finale — G major pentatonic grand arpeggio** — 8-note G4→D6 arpeggio (sine + triangle alternating for bell-tower warmth), a sustained G5 pedal root, and a finale sub-bass 90→45 Hz opening thump. G major pentatonic is the brightest of the moon-festival tonal palettes: harvest moon (warm amber), snow moon (A minor pentatonic triangle — cool crystalline), cherry moon (D major pentatonic sine — warm soft), summer fireworks (G major pentatonic sine+triangle — brightest & most celebratory). The four seasonal-night events now carry four distinct musical palettes.
- **Bloom visual** — three-layer sprite: (1) a radial gradient colored glow at the center, (2) N radial spokes (5/6/8 per-bloom) drawn as tapered gradient triangles, and (3) a bright white-hot central core kernel. Cheered blooms gain an extra outer halo ring in the firework's hue color. Each bloom expands over ~0.5s, holds bright for ~4s, then fades over ~1.5s.
- **Particle choreography** — on bloom, ~3 colored-confetti particles per spoke radiate outward at staggered speeds (1.5, 2.0, 2.5 px/frame) creating streaking trails, plus 6 big white sparkle kernels in the center for the flash. On cheer, 12 confetti particles in the firework's hue burst outward + 2 drifting hearts float up. On blessing, 5 rainbow bursts across the sky (16 confetti each in each hue) + 24 drifting bright sparkles wash the upper canvas.
- **Two-layer drawing** — rockets drawn at the sky layer (behind the pet) so a rising rocket visibly streaks up from behind the pet silhouette, then the bloom itself draws at the upper-air layer (above the pet) so the starburst reads as floating high in the sky. Small detail that makes the show feel spatially deep.
- **Generous ~20 px cheer hitbox** — bigger than the snow moon's snowflakes (~9 px) or the cherry moon's cranes (~10.5 px). Fireworks are big bright targets, and the cheer window only lasts ~4s, so the hitbox leans toward forgiving.
- **"cheer ✨" hint** — blinks above the first un-cheered visible bloom until the player has completed their first blessing, then disappears forever. Matches the hint pattern of mooncake / snowflake-keepsake / fairy / bunny / cardinal / cherry-moon / sakura-petal-keepsake.
- **Post-blessing afterglow** — a soft rainbow haze (red → violet → transparent) washes the upper canvas for ~3s after the blessing, plus continuous ambient confetti/sparkle emission shedding from random sky positions for ~3 seconds — the "festival isn't quite over" beat.
- **Graceful timeout** — if the player misses cheering some fireworks, the event fades out gracefully without a blessing after all rockets have bloomed + ~6s grace. No punishment, just a quiet closing.
- **Natural lifespan** — ~60s max from spawn to auto-fade. Event also ends early if weather turns stormy/rainy, if time advances past night, if the season changes out of summer, or if the pet falls asleep (unless the blessing is already playing).
- **Sighting speech (90% chance on spawn)** — 6 variants ("Fireworks~! 🎆✨", "Hanabi night~! 🎆", "Summer fire flowers~! 🎆🔥", "The sky is dancing~! 🎆💖", etc.).
- **Cheer speech (50% chance per cheer)** — 5 variants including "Tamaya~! 🎆" (the traditional cheer).
- **Bless speech (100% on blessing)** — 5 celebratory variants ("The summer sky blooms for me~! 🎆💖", "Every firework cheered~! Tamaya! 🎆✨", etc.).
- **Achievement #80 "Summer Fireworks Blessed"** — cheer for every firework in a summer hanabi show (🎆, scaled to match Cherry Moon Blessed / Snow Moon Blessed / Moon Blessed since summer hanabi are equally rare).
- **Diary milestones** — first sighting (🎆) + first blessing (🎆) + ongoing "Summer fireworks blessing #N~! Tamaya!" entries per subsequent blessing.
- **"summer_fireworks" dream template** — sleeping after the festival yields dreams of "a sky of blooming fireworks~", "colored stars exploding~", "hanabi festival forever~".
- **Sleep-talks** — 3 new contextual mumbles: "*mumble*... pretty colors... in the sky...", "Zzz... tamaya... hehe... more...", "*snore*... bright bloom... bigger bigger...".
- **Contextual dream icons** — `dailyActivityLog` now recognizes "summer_fireworks" and biases post-event dream icons toward star/heart (bright celebratory iconography).
- **Stats tracking** — `🎆 N blessed (M cheered)` displayed in the WEATHER section of the stats panel in warm gold `#FFD07A` between SAKURA PETALS and LIGHTNING BOLTS.
- **Full persistence** — `totalSummerFireworksSeen`, `totalFireworksCheered`, `totalSummerFireworksBlessed`, `summerFireworksFirstSeen`, `summerFireworksFirstBlessed` saved across sessions.
- **Total achievements: 80.**
- Renderer grew from ~28,164 to ~28,796 lines (+~632 lines). Single-function insertion block right after `drawSakuraPetalKeepsake` with all feature-scoped state, constants, audio, logic, and rendering.

**Why this feature:** Three of the four seasonal-night events shipped in prior cycles — autumn's Harvest Moon Festival + mooncake keepsake, winter's Snow Moon Festival + snowflake keepsake, spring's Cherry Moon Festival + sakura petal keepsake. The last remaining gap, flagged as the #1 priority in every NEXT.md for the past 5+ cycles, was summer-night. The previous cycle's notes specifically called it out as "the last remaining seasonal-night gap" and the highest-value next feature, listing four candidate options (fireworks, firefly sea, meteor shower, paper lanterns on a river). Fireworks (hanabi) is the single most iconic Japanese summer-night tradition — no other summer-night signature carries the cultural weight that hanabi does, with its own traditional cheer ("tamaya!"), its own sub-genres (peony-shape vs. chrysanthemum-shape vs. willow-shape blooms — reflected in the 5/6/8 spoke counts), and its own musical tradition. Shipping it completes the **four-season seasonal-night vocabulary**: autumn amber harvest, winter silver snow, spring pink cherry, summer rainbow hanabi. Each has a distinct palette, a distinct musical key, a distinct interaction verb (light / catch / wish / cheer), and a distinct emotional register. Mechanically the fireworks festival mirrors the proven 4-stage template (rise → interact → bless → [optional keepsake drop]) but introduces new vocabulary point-for-point rather than reskinning existing elements: a *staggered* multi-rocket show rather than a single big object + satellites; a *timed cheer window* per bloom rather than an always-clickable satellite; a *loud celebratory* audio palette (whistle → BWOOM → bright chime → grand arpeggio with sub-bass) rather than the quieter audio palettes of the other three moons; a *rainbow* color palette rather than a single-hue palette; *N-point starburst* shapes (5/6/8) with procedural spoke-count variety rather than identical sprites. The verb *cheer* is the fourth distinct seasonal-night verb (after light, catch, wish) — a loud-out-loud verb that contrasts especially sharply with the cherry moon's quiet wish, perfectly matching the contrast between a quiet spring hanami night and a raucous summer hanabi festival. Having all four seasons with their own signature night event now means a player who plays a full year will witness four entirely different celestial celebrations with matched daytime parity — a complete annual cycle. A keepsake drop for the summer fireworks festival (candidate: a glass firework-fragment charm, a paper-cone firework stub, a sparkler memento) is reserved for the next cycle, mirroring the cherry moon → sakura petal keepsake 2-cycle cadence from the past two releases.
...
### Added
- **Sakura petal keepsake drop** — after every Cherry Moon Blessing, the pink moon now drops a single oversized 5-petal cherry blossom keepsake as a spring gift. The keepsake spawns at the moon's position and tumbles down to the ground with real-petal physics: horizontal flutter (amplitude tapers as it approaches the ground so the landing is precise — 8 px flutter vs snowflake's 6 px sway), continuous rotation during the fall (`spin += 0.08/frame` — visibly faster than snowflake's subtle 0.05 because petals tumble more than crystals drift), and a gentle accelerating descent over ~3 seconds (longer than snowflake's 2.3s and mooncake's 1.5s — petals flutter down slowest of all). On landing, a soft paper-whisper rather than a crystalline tink plays with a 6-particle blossom-puff (mostly pink petals, some sparkles) and a bounce-in squash (1.25× horizontal / 0.80× vertical, eases back over ~28 frames).
- **Warm descending sine cascade drop chime** — warm sine-wave 4-tone cascade (D6 → A5 → F#5 → D5, ~0.11s between notes, ~0.4s per note) plus a warm low-register C#7 sine shimmer harmonic lasting the entire fall. Distinct from the snowflake's bright triangle-wave E7→C7→A6→F6 glockenspiel: the petal's cascade is warmer, rounder, lower, and uses pure sines rather than triangles, and its shimmer sits at C#7 rather than the snowflake's high B7 — the whole drop chime says "soft spring night" rather than "winter crystal".
- **Soft paper-whisper landing sound** — rather than the snowflake's crystalline triangle tink, a petal lands with a gentle paper sigh. A warm E5 sine with a subtle slide to C5 (0.12s) plus a low-pass-filtered (~1400 Hz) pink-noise tail for paper-rustle texture. The inverse of snowflake's high-filtered frost-tinkle.
- **Oversized 5-petal cherry blossom sprite** (~14 px diameter) — 5 rounded teardrop petals at 72° intervals around a warm cream-yellow pollen center, each petal with a radial-outward gradient shadow edge for depth and a subtle notch at its tip (the signature sakura silhouette — cherry petals have that characteristic inverted-V cleft). Warm cream-yellow pollen center with 6 tiny darker stamen dots radiating from center, plus a bright inner core glow. Rendered in one of the 4 cherry-moon crane palettes (soft cherry pink 340°, warm peach 15°, paper white 0° sat 0, deeper rose 330°) randomly picked per drop — visually ties the keepsake back to the cranes the player just wished on. Regardless of petal hue, the pollen stays warm cream-yellow for a unified sakura feel.
- **Warm pink blush halo pulse** — while resting on the ground, a warm halo in the petal's hue (linear-gradient `hsla(hue, 75%, 88%, 0.5)` → transparent) pulses at `sin(glowPulse)` at 10-13.5 px radius, inviting the player to notice and click. Replaces the snowflake's cool silvery-blue halo with the cherry moon's pink palette.
- **Persistent ambient petal emission** — while landed, the keepsake emits a tiny particle every 45-80 frames drifting gently upward, and 55% of the time that particle is a *pink petal* rather than a sparkle (a cherry-moon keepsake sheds petals, not frost). This contrasts with the snowflake's pure-sparkle ambient emission, reinforcing the visual vocabulary of each keepsake.
- **Gentle "press 🌸" hint** — faint blinking label floats above the keepsake until the player has pressed their first one, then disappears forever (matching the hint pattern of mooncake/snowflake-keepsake/fairy/bunny/cardinal/cherry-moon).
- **Click to press** — generous ~10.5 px circular hitbox (slightly larger than snowflake's 10 px since the 5-petal blossom has a wider silhouette); clicking the landed keepsake *presses* the petal (like pressing flowers in a book, keeping it forever). Plays a warm ascending F# major sine arpeggio (F#5-A5-C#6-F#6, ~0.08s between notes, ~0.48s per note) + C#7 sine shimmer tail (matching the cherry moon blessing's top note). 14 pink sparkle particles + 4 drifting blossom petal particles + 3 heart particles float upward, +5 happiness / +2 care points / +4 friendship XP, and a contextual press speech ("I'll press this forever~! 🌸💖", "A cherry moon keepsake~! 🌸✨", "My pink memento~! 🌸✨").
- **Memento, not food** — like the snowflake keepsake (and unlike the mooncake), the pressed petal is a *memory keepsake* and grants no hunger at all, just happiness/care/XP. Keepsake category stays coherent: harvest-moon → mooncake (food/feast), snow-moon → snowflake keepsake (memento), cherry-moon → pressed petal (memento). Two out of three seasonal keepsakes are now mementos, with the mooncake remaining the only caloric reward.
- **New verb: *press*** — third distinct keepsake-interaction verb. Mooncake is *shared* (food ritual), snowflake is *treasured* (preserve crystal), sakura petal is *pressed* (pressed between pages like a dried-flower album). Mirrors the hanami/pressed-flower tradition where cherry blossoms are preserved as keepsakes of a single spring. In a game that already has many food-sharing interactions, the *press* verb opens yet another interaction category distinct from both share and treasure.
- **Natural lifespan** — keepsake sits on the ground for ~40 seconds before fading out over 60 frames if un-pressed, matching the snowflake. Fades out faster if the pet goes to sleep while a keepsake is still on the ground.
- **70% chance of sighting speech** — 70% of keepsake drops trigger a contextual sighting speech ("A sakura petal keepsake~! 🌸", "Whooa~ a gift from the cherry moon! 🌸🌕", "A tiny pink blossom~! 🌸✨") so each drop isn't silent but also doesn't always repeat the same line.
- **First-press diary milestone** — a dated diary entry records the first time a petal is pressed ("The cherry moon gave me a sakura petal keepsake~! I pressed it to keep forever! 🌸🌕✨"). Subsequent presses add shorter "Pressed sakura petal keepsake #N~!" entries.
- **Sakura Petal Keeper achievement** — press a sakura petal keepsake from a blessed cherry moon to unlock achievement #79 (🌸). Single-completion achievement scaled to match the Snowflake Keeper tier, since cherry-moon blessings are equally rare.
- **Dream template** — "sakura_petal" added to `DREAM_TEMPLATES` so sleeping after pressing a petal yields dreams of "a book of pressed petals~", "sakura blossoms in my paws~", "the moon's soft pink gift~".
- **Sleep-talk phrases** — 3 new contextual sleep talks: "*mumble*... tiny petal... pressed forever...", "Zzz... keepsake from the moon... hehe...", "*snore*... pink blossom... safe with me...".
- **Contextual dream icons** — `dailyActivityLog` now recognizes "sakura_petal" and biases dream icons toward flower/heart/moon after a petal is pressed.
- **Stats tracking** — total sakura petals pressed + total dropped displayed in the WEATHER section of the stats panel in a soft `#FFD6E0` SAKURA PETALS row between CHERRY MOON and LIGHTNING BOLTS.
- **Full persistence** — totalSakuraPetalKeepsakesPressed, totalSakuraPetalKeepsakesDropped, and sakuraPetalKeepsakeFirstPressed saved across sessions.
- **Total achievements: 79**

**Why this feature:** Last cycle (v0.103 Cherry Moon Festival) shipped the spring-night blessing payoff — warm D-major pentatonic arpeggio + pink sparkle/petal fountain + spring-breeze noise — but stopped at the blessing. NEXT.md's *very first* "Thoughts for next cycle" entry called this out directly: *"Cherry blossom / sakura petal keepsake drop — the natural follow-up to complete the spring-night 4-stage arc (rise → interact → bless → collectible drop). After the blessing, a single larger pink cherry-blossom petal (or a pink-paper origami heart, or a folded paper crane keepsake) arcs down from the moon and lands as a persistent ground collectible to treasure with the pet. Would fully close the harvest → snow → cherry seasonal-parity pattern and reuse the proven keepsake template. Feels like the most natural immediate follow-up."* That's exactly the gap this cycle fills, and it completes a beautiful three-of-four seasonal-keepsake parity pattern: autumn-night = harvest moon + lanterns + *mooncake* (shared feast); winter-night = snow moon + snowflakes + *snowflake keepsake* (treasured crystal); spring-night = cherry moon + cranes + *sakura petal keepsake* (pressed petal). A player who blesses every seasonal moon now receives a different kind of gift from each, with three different interaction verbs. Mechanically the sakura petal mirrors the snowflake keepsake's proven "falls from moon → drifts to ground → click to receive" structure but inverts the vocabulary point-for-point so they feel distinct rather than interchangeable reskins: snowflake's crystalline 6-armed radial snowflake → 5-petal rounded cherry blossom with a yellow-cream pollen center; snowflake's side-to-side sine sway → wider petal flutter *plus* faster continuous rotation (0.08 vs 0.05) because real petals tumble more visibly through the air than crystals drift; snowflake's cool E7→C7→A6→F6 *triangle-wave glockenspiel* cascade → warm D6→A5→F#5→D5 *sine-wave* cascade that sits a full octave lower in a warmer register; snowflake's high B7 shimmer harmonic → warm C#7 shimmer that matches the cherry moon blessing's top note; snowflake's crystalline triangle "tink" + 4500 Hz bandpass frost-noise landing → soft sine E5→C5 sigh + 1400 Hz low-pass paper-rustle landing (a petal lands with a whisper, not a tink); snowflake's cool C-E-G-C triangle-wave silver arpeggio press-sound → warm F#-A-C#-F# sine-wave arpeggio press-sound (F# major to mirror the cherry moon blessing's D-major palette); snowflake's cool silvery-blue halo palette → warm pink blush halo palette (both track their moon's palette); snowflake's pure sparkle-only ambient emission → *petal-biased* ambient emission where 55% of emitted particles are pink petals rather than sparkles. The "press" verb (rather than "treasure" again) was chosen deliberately to introduce a third distinct keepsake-interaction category, tying directly into hanami tradition where cherry blossoms are pressed in books to preserve a single spring forever. And color-wise the petal picks a random palette from the same 4-hue set as the cranes the player just wished on, so visually the keepsake reads as a fallen sibling of the cranes, the connection between catch and reward reinforced through color continuity. Structurally this also finishes the 4-stage template across all three seasonal moons, proving the pattern is solid and ready for the last remaining seasonal-night slot (summer-night — fireworks? shooting-star meteor shower? sea of fireflies? tide-pool paper lanterns?). Once summer-night is shipped with its own 4-stage arc, the game will have a complete year-round seasonal-moon vocabulary spanning daytime and nighttime, each with a keepsake payoff.

## [v0.103.0] — 2026-04-20 — Cherry Moon Festival

### Added
- **Cherry Moon Festival (桜月夜 / sakura-zukiyo)** — a rare spring-night signature event where an oversized soft pink-peach full moon rises into the sky alongside 4-6 origami paper cranes (orizuru 折鶴) floating in the upper air with slow wing-beats, waiting to have wishes made on them. Fills the long-standing "spring has no night signature event" gap that prior NEXT.md cycles kept flagging as the last remaining major seasonal gap — spring now has a daytime cherry-blossom petal festival AND a signature night moment, matching autumn (harvest moon) and winter (snow moon) parity. Spawn rate ~0.00012/frame during spring night in clear/cloudy/rainy (non-stormy) weather with no existing harvest or snow moon active.
- **Oversized pink-peach moon** — rendered with a radial gradient from cream-pink `#FFF0F4` center to deep rose `#B6687E` rim, a **subtle moon-rabbit silhouette** on its face (a soft-shadow callback to the East Asian folktale of the rabbit pounding mochi on the moon: body + head + two tall tilted ears in semi-transparent rose-brown), a small additional crater on the right, upper-left specular highlight in cream-pink, and a breathing outer halo in pink blush pulsing at `sin(frame * 0.015)` — slower than snow moon's 0.018 for a calmer spring feel.
- **Moon rise animation** — the moon starts 50 px below its resting position and eases upward with a cubic ease-out over 10 seconds, so you watch it rise into the spring night sky. Moon spawns with a right-bias (`w * 0.55-0.75`) to spatially distinguish from the snow moon's left-bias and the harvest moon's far-right bias — cherry sits closer to center-right than harvest's edge.
- **4-6 origami paper cranes (orizuru)** — float freely in the upper air with slow breathing wing-beats (instead of snow moon's free-drifting rotating snowflakes, or harvest moon's swinging lanterns). Each crane is drawn in traditional paper-fold silhouette: central kite-shaped body, upper wing triangular folded-paper shape, lower wing mirror, long forward head + beak line with tiny head dot at the tip, long pointed tail behind, and visible fold-line details across each wing.
- **Independent bob + wing-beat** — each crane has its own `bobPhase/bobSpeed` for gentle sine-wave drift (±3 px x, ±2 px y) and `wingPhase/wingSpeed` for per-crane wing-open/close oscillation (0.028-0.048 rad/frame) so the flock isn't visually synchronized and each crane feels hand-folded.
- **4 crane color variants** — randomized per bird: soft cherry-pink (hue 340), warm peach (hue 15), paper-white (hue 0 sat 0), deeper rose-pink (hue 330). Un-wished cranes render as muted paper-cream `rgba(240, 230, 230, 0.78)`; wished cranes bloom into full-color glow with pulsing halo + inner warm core shimmer.
- **Click to wish** — click an un-wished crane (~10.5 px circular hitbox — slightly larger than snow moon's 10 px since cranes have more detailed silhouettes) to "wish" on it: gentle wooden koto-like C6 + G6 pluck (sine + triangle, a sweet open fifth) + C#7 sine shimmer harmonic, 10 pink sparkle particles + 2 small cherry-blossom petal particles bloom at the wish point, the crane transitions from muted paper-cream into full-color glow with a pulsing halo + inner core over ~25 frames (`wishAnim 0→1` at +0.04/frame) + gentle upward liftY drift (up to 4 px higher, as if the wish carries the crane slightly skyward), +1 happiness / +1 friendship XP, 50% chance of speech ("Made a wish~! 🕊️", "Ooh, a crane~! 🕊️✨").
- **Wished-crane ambient sparkles** — once wished, each crane emits a tiny sparkle particle near its position every 55-90 frames drifting slightly upward, so the lit sky has a subtle ongoing shimmer — continuing the "wishes taking flight" metaphor.
- **Cherry Moon Blessing** — wishing on *every* crane triggers the big festival payoff: warm **D major pentatonic arpeggio** (D5-E5-F#5-A5-B5-D6-F#6 — 7 notes of warm sine waves, a shade rounder than snow moon's 6 triangle-wave A-minor notes) + C#7 sine shimmer on top + **soft low-filtered pink-noise "spring breeze" tail** (1200 Hz lowpass) for a warm close-breath feel rather than snow moon's high crystalline wind-whisper, 32-particle pink sparkle fountain + 14 pink-blossom petal particles radiating from the moon (vs snow moon's 10 drifting snowflake particles), big pink glow halo pulses around the moon for ~12s, continuous ambient petal/sparkle emission from the moon for the first 2 seconds, and +5 happiness / +3 care / +5 friendship XP. Special celebration speech ("The cherry moon blesses me~! 🌕🌸", "Every crane carried a wish~! 🕊️🌸").
- **Festival lifecycle** — natural 2-minute lifespan; fades in over 180 frames (3s) and fades out over 180 frames (3s). If time turns to morning, season changes away from spring, weather turns stormy, or the pet goes to sleep while the festival is NOT yet blessed, it gracefully fades out early. Blessed festivals finish playing out their magic undisturbed.
- **Gentle hint text** — faint blinking "wish 🕊️" label above the first un-wished crane until the player has blessed their first cherry moon festival, then disappears forever (matching the hint pattern used by mooncake/fairy/bunny/cardinal/harvest-moon/snow-moon).
- **First-sighting & first-blessing diary milestones** — two separate dated diary entries record the first time a cherry moon ever appears ("A pink cherry moon rose over my world~! Tiny paper cranes floated in the sky, waiting for my wishes! 🌕🌸🕊️") and the first time all cranes are wished upon ("I wished on every crane under the cherry moon~! The sky answered with a pink spring blessing! 🌕🌸🕊️"). Subsequent blessings add shorter "Cherry moon blessing #N~!" entries.
- **Dream template** — added "cherry_moon" to `DREAM_TEMPLATES` so sleeping after a cherry moon session yields dreams of "a sky of paper cranes~", "moonlit hanami festival~", "wishing under the pink moon~".
- **Sleep-talk phrases** — 3 new contextual sleep talks for cherry_moon: "*mumble*... pink moon... so soft...", "Zzz... paper cranes... wishing...", "*snore*... hanami... under the stars...".
- **Contextual dream icons** — `dailyActivityLog` now recognizes "cherry_moon" and biases dream icons toward flower/moon/heart after wishing on cranes.
- **Cherry Moon Blessed achievement** — wish on every crane under a cherry moon to unlock achievement #78 (🌕). Single-completion achievement scaled to match Moon Blessed (harvest moon) and Snow Moon Blessed (winter), since cherry moons are equally rare.
- **Stats tracking** — total cherry moons blessed + total cranes wished displayed in the WEATHER section of the stats panel between SNOWFLAKE KEEPSAKES and LIGHTNING BOLTS in a bright `#FFC6D8` CHERRY MOON row.
- **Full persistence** — totalCherryMoonsSeen, totalCranesWished, totalCherryMoonsBlessed, cherryMoonFirstSeen, and cherryMoonFirstBlessed saved across sessions.
- **Total achievements: 78**

**Why this feature:** Last cycle (v0.102 Snowflake Keepsake Drop) completed winter's full night-signature arc (rise → catch → bless → collectible drop), matching autumn's harvest-moon arc point-for-point. NEXT.md's *very first* "Thoughts for next cycle" entry opened with: *"Spring night signature event — cherry-blossom moon? Fireflies forming short-lived constellations that trace out a familiar constellation once caught? Matches autumn's harvest moon + winter's snow moon. Spring still has mostly day features and is the biggest remaining night gap now that winter is filled."* That's exactly the gap this cycle fills, and I picked the cherry-blossom-moon option because it creates a clean three-of-four seasonal parity pattern the player can learn once and recognize across seasons: autumn-night = harvest moon + lanterns + mooncake; winter-night = snow moon + snowflakes + snowflake-keepsake; spring-night = cherry moon + cranes (+ future keepsake in a later cycle, following the proven 4-stage template). Today's date (2026-04-20) also happens to be peak spring/hanami season, so shipping the cherry moon festival today feels seasonally right. Mechanically the cherry moon mirrors the snow moon's proven multi-stage structure (rise → interact → bless) but inverts the vocabulary point-for-point so they feel distinct rather than interchangeable reskins: snow moon is *silvery-blue* for winter stillness, cherry moon is *pink-peach* for spring warmth; snow moon's sky element is *free-drifting rotating snowflakes* (gravity-neutral crystals), cherry moon's is *floating paper cranes* (gentle wing-beating birds) that physically *lift slightly higher* after being wished upon as if the wish carries them upward; snow moon's catch-sound is a *crystalline E6+B6 triangle-wave frost-tinkle*, cherry moon's wish-sound is a *warm C6+G6 sine+triangle koto-like pluck* (a sweet open fifth rather than a bright open twelfth); snow blessing is *cool A-minor pentatonic triangle waves + high bandpass wind-whisper*, cherry blessing is *warm D major pentatonic sine waves + low-filtered spring-breeze pink noise* (a close warm breath through petals rather than a high crystalline winter wind). The cherry moon also spatially biases to the right (w * 0.55-0.75) to distinguish from snow moon's left-bias and harvest moon's far-right edge — across a full four-season year of play, a returning player will notice all four moons occupy slightly different celestial positions, an implicit visual rhythm across the seasons. Two new subtle touches unique to this event: (1) the **moon-rabbit silhouette** on the moon's face — a soft-shadow body + head + two tall tilted ears drawn in semi-transparent rose-brown, a cultural callback to the East Asian folktale where a rabbit pounds mochi on the moon's surface, reading as abstract crater detail from a distance and as a rabbit on closer look; and (2) the **wish-lifts-the-crane** mechanic — unlike snow-catch which just freezes a snowflake in place, a wish *physically lifts* the crane up to 4 px higher as if the wish carries it skyward, reinforcing the "crane as a vessel for a wish" metaphor that's core to Japanese origami tradition (senbazuru — folding 1000 paper cranes for a wish). Cranes in folklore are symbols of hope, healing, and wishes, so the cherry moon becomes a spring "wish moon" rather than a second catch-em-all festival, opening a new verb category (*wish*) distinct from snow moon's *catch* verb and the mooncake's *share* verb and the keepsake's *treasure* verb. Thematically this completes three of four seasonal night events: autumn has its harvest moon, winter its snow moon, spring now its cherry moon, and NEXT.md's future leaps can target summer (fireworks? shooting-star meteor shower? sea of fireflies over a tide pool?) knowing the 3-of-4 template is solid and the pattern is proven.

## [v0.102.0] — 2026-04-19 — Snowflake Keepsake Drop

### Added
- **Snowflake keepsake drop** — after every Snow Moon Blessing, the silver moon now drops a single oversized crystalline snowflake keepsake as a winter gift. The keepsake spawns at the moon's position and drifts down to the ground with a real-snowflake sway: horizontal sine drift (amplitude tapers as it approaches the ground so the landing is precise), slow rotation during the fall (`spin += 0.05/frame`), and a feather-light accelerating descent over ~2.3 seconds (slower than the mooncake's 1.5s drop — snowflakes fall gently). On landing, a tiny crystalline "tink" plus a high-filtered frost-noise tail plays with a 6-particle frost puff and a bounce-in squash (1.25× horizontal / 0.80× vertical, eases back over ~28 frames).
- **Descending glockenspiel drop chime** — cool glockenspiel-style descending twinkle (E7 → C7 → A6 → F6 triangle-wave tones, ~0.09s between notes, ~0.34s per note) plus a soft B7 sine-shimmer harmonic lasting the entire fall. Distinct from the mooncake's warm G6→E6→C6→A5 sine glissando: the snowflake's is higher, cooler, triangle-wave sharp rather than sine-wave warm, and the B7 shimmer tail runs the entire fall to give it a sustained crystalline sparkle.
- **Crystalline "tink" landing sound** — rather than the mooncake's warm low-sine thump, a feather-light snowflake shouldn't thud. The keepsake lands with a short two-step triangle tink (C7 → G6 over 0.12s) plus a high-filtered noise tail (~4500 Hz bandpass, 0.12s) for frost texture.
- **Oversized crystalline snowflake sprite** (~14 px diameter) — 6-armed radial snowflake with two pairs of perpendicular branches at 35% and 65% along each arm (different from the smaller 40%/70% sky snowflakes, making the keepsake read as a bigger, more detailed cousin), diamond tips at each arm's end, hexagonal center hub, bright inner core radial gradient, and a soft inner glow disc behind the crystal. Rendered in one of the 4 sky-snowflake color palettes (icy-blue hue 200, pale-lavender hue 260, pearl-white hue 0 sat 0, frosty-mint hue 170) randomly picked per drop — visually ties the keepsake back to the sky snowflakes the player just caught.
- **Cool silvery-blue halo pulse** — while resting on the ground, a cool halo in the snowflake's hue (linear-gradient `hsla(hue, 70%, 90%, 0.5)` → transparent) pulses at `sin(glowPulse)` at 10-13.5 px radius, inviting the player to notice and click. Replaces the mooncake's warm amber halo with the snow moon's palette.
- **Persistent ambient sparkles** — while landed, the keepsake emits a tiny sparkle particle from its rim every 40-75 frames rising gently upward, so the ground gift keeps glimmering rather than sitting still. First reward-drop in the game with ongoing ambient emission (mooncake sits still with only a pulsing halo).
- **Gentle "treasure ❄️" hint** — faint blinking label floats above the keepsake until the player has treasured their first one, then disappears forever (matching the hint pattern of mooncake/fairy/bunny/cardinal/harvest-moon).
- **Click to treasure** — generous ~10 px circular hitbox; clicking the landed keepsake *treasures* it with the pet: ascending crystalline silver arpeggio (C6-E6-G6-C7 triangle waves, ~0.075s between notes, ~0.42s per note) + B7 sine shimmer tail, 14 silvery sparkle particles + 4 drifting snowflake particles + 3 heart particles floating upward, +5 happiness / +2 care points / +4 friendship XP, and a contextual treasure speech ("I'll treasure this forever~! ❄️💖", "A snow moon keepsake~! ❄️✨", "My silver memento~! ❄️✨").
- **Memento, not food** — unlike the mooncake (a feast that grants +20 hunger), the snowflake keepsake is a *memory keepsake* and grants no hunger at all, just happiness/care/XP. This mechanically differentiates the two reward drops so they feel like different categories of gift: one a harvest-night feast, the other a winter crystal-memory. Harvest nostalgia is warm and caloric; winter stillness is cool and contemplative.
- **Natural lifespan** — keepsake sits on the ground for ~40 seconds before fading out over 60 frames if un-treasured, matching the mooncake. Fades out faster if the pet goes to sleep while a keepsake is still on the ground.
- **70% chance of sighting speech** — 70% of keepsake drops trigger a contextual sighting speech ("A snowflake keepsake~! ❄️", "Whooa~ a gift from the snow moon! ❄️🌕", "A tiny silver star~! ❄️✨") so each drop isn't silent but also doesn't always repeat the same line.
- **First-treasure diary milestone** — a dated diary entry records the first time a keepsake is treasured ("The snow moon gave me a snowflake keepsake~! I'll treasure it forever! ❄️🌕✨"). Subsequent treasures add shorter "Treasured snowflake keepsake #N~!" entries.
- **Snowflake Keeper achievement** — treasure a snowflake keepsake from a blessed snow moon to unlock achievement #77 (❄️). Single-completion achievement scaled to match the Mooncake Keeper tier, since snow-moon blessings are equally rare.
- **Dream template** — "snowflake_keepsake" added to `DREAM_TEMPLATES` so sleeping after treasuring a keepsake yields dreams of "a crystal shelf of keepsakes~", "silver snowflakes in my paws~", "the moon's gift glowing softly~".
- **Sleep-talk phrases** — 3 new contextual sleep talks: "*mumble*... tiny crystal... mine forever...", "Zzz... keepsake from the moon... hehe...", "*snore*... silver snowflake... safe with me...".
- **Contextual dream icons** — `dailyActivityLog` now recognizes "snowflake_keepsake" and biases dream icons toward heart/star/moon after a keepsake is treasured.
- **Stats tracking** — total snowflake keepsakes treasured + total dropped displayed in the WEATHER section of the stats panel in a soft `#D6E6FA` SNOWFLAKE KEEPSAKES row between SNOW MOON and LIGHTNING BOLTS.
- **Full persistence** — totalSnowflakeKeepsakesTreasured, totalSnowflakeKeepsakesDropped, and snowflakeKeepsakeFirstTreasured saved across sessions.
- **Total achievements: 77**

**Why this feature:** Last cycle (v0.101 Snow Moon Festival) shipped the blessing payoff — silver pentatonic arpeggio + sparkle fountain + wind-whisper — but stopped there. NEXT.md's *very first* "Thoughts for next cycle" entry called this out directly: *"Snowflake keepsake drop — after the blessing, a snowflake-crystal keepsake arcs down from the moon (like mooncake from harvest moon) and lands as a persistent collectible the pet can nibble or carry. This completes the parity — harvest-moon → mooncake, so snow-moon → snowflake keepsake. Feels like the most natural immediate follow-up."* That's exactly the gap this cycle fills, and it completes a beautiful seasonal-parity pattern: autumn-night = harvest moon + lanterns + mooncake reward; winter-night = snow moon + snowflakes + snowflake-keepsake reward. The next player to catch every snowflake and receive the blessing will now also get a falling crystal gift — the multi-stage festival payoff arc (rise → interact → bless → receive keepsake) is finally complete for both moon events. Mechanically the keepsake mirrors the mooncake's proven "falls from moon → arcs to ground → click to receive" structure but inverts the vocabulary point-for-point so they feel distinct rather than reskins: mooncake *rotates rapidly* on a tight parabolic arc with a small arc-hump, the keepsake *drifts side-to-side with sway* like a real snowflake on a gentler accelerating fall; mooncake lands with a warm low-sine *thump* because a pastry has mass, the keepsake lands with a crystalline triangle *tink* + frost-noise tail because a snowflake is nearly weightless; mooncake has a warm amber halo of invitation, the keepsake has a cool silver-blue halo matching its moon's palette; mooncake is *food* that grants +20 hunger as well as happiness/care/XP, the keepsake is a *memento* granting only happiness/care/XP so the two reward types feel like different categories of gift (warm harvest feast vs. cool winter crystal-memory). The keepsake also introduces a small new pattern: *persistent ambient sparkle emission while landed* — the mooncake sits still with only a pulsing halo, but the keepsake keeps emitting faint rising sparkles from its rim so the ground gift feels alive rather than static. Color-wise the keepsake picks a random palette from the same 4-hue set as the sky snowflakes, so visually it reads as a bigger cousin of the crystals the player just caught — the connection between the catch phase and the reward phase is reinforced through color continuity rather than just narrative. The "treasure" verb (rather than "share" or "eat") was chosen deliberately to mark this as a keepsake action rather than a food action — in a game that already has many food-sharing interactions, a treasure-a-memento action opens a new interaction category that future keepsake-style drops (fairy petal, cardinal feather, acorn hoard trinket) could follow.

## [v0.101.0] — 2026-04-19 — Snow Moon Festival

### Added
- **Snow Moon Festival** — a rare winter-night special event where an oversized pale silvery-blue full moon rises into the sky alongside 4-6 paper snowflakes drifting in the upper air, waiting to be caught. The festival lasts ~2 minutes and only appears on winter nights in clear/cloudy/snowy (non-stormy/non-rainy) weather. Fills the long-standing "winter night has no signature event" gap that prior NEXT.md cycles kept flagging as the biggest remaining winter gap — winter now has a daytime cardinal, daytime bunny, AND a signature night moment, matching autumn's harvest-moon parity.
- **Oversized silvery-blue moon** — significantly larger than typical night-sky elements (radius ~22-26 px), rendered with a radial gradient from pale ice `#F2F8FF` center to deep slate-blue `#4A6890` rim, three darker cool-toned crater details, upper-left specular highlight, and a breathing outer silvery halo pulsing at `sin(frame * 0.018)`. Slightly slower pulse rate than the harvest moon's `0.02` so the snow moon reads as cooler and more contemplative.
- **Moon rise animation** — the moon starts 50 px below its resting position and eases upward with a cubic ease-out over 10 seconds, so you watch it rise into the winter sky. Moon spawns with a slight left-bias (`w * 0.18-0.36`) to spatially distinguish it from the harvest moon's right-bias.
- **4-6 paper snowflake crystals** — drift freely in the upper air without hanging strings (unlike the harvest moon's tethered lanterns). Each crystal is a 6-armed radial paper snowflake with two pairs of perpendicular branches at 40% and 70% along each arm, tiny diamond tips at the end of every arm, and a hexagonal center hub. Uncaught flakes are muted paper-grey (`rgba(200, 210, 225, 0.72)`); caught flakes bloom in full color with a glowing halo.
- **Independent drift + rotation** — each snowflake has its own `bobPhase/bobSpeed` for gentle sine-wave drift (±3 px x, ±2 px y) and `rotation/rotSpeed` for slow per-flake angular rotation (0.003-0.008 rad/frame, random clockwise or counter-clockwise), so the set feels hand-cut and alive rather than uniformly grid-placed.
- **4 snowflake color variants** — randomized per crystal: icy-blue (hue 200), pale-lavender (hue 260), pearl-white (hue 0 sat 0), frosty-mint (hue 170). Each variant tints the bloom color, edge color, center hub color, and inner-core glow after catching.
- **Click to catch** — click an uncaught snowflake (~10 px circular hitbox) to catch it: crystalline two-tone E6+B6 triangle-wave frost-chime + B7 shimmer harmonic, 10-particle icy sparkle bloom, the snowflake "freezes" from muted paper-grey into full-color glow with a pulsing halo + inner bright core over ~25 frames (`catchAnim 0→1` at +0.04/frame), +1 happiness / +1 friendship XP, 50% chance of speech ("Caught one~! ❄️", "Ooh, a crystal~! ❄️✨").
- **Caught-snowflake ambient sparkles** — once caught, each snowflake emits a tiny sparkle particle near its position every 50-80 frames, so the lit sky has a subtle ongoing shimmer.
- **Snow Moon Blessing** — catching *every* snowflake triggers the big festival payoff: cool A-minor pentatonic arpeggio (A5-C6-E6-A6-C7-E7) + B7 crystalline shimmer on top + high-filtered noise "wind whisper" for winter flavor, 32-particle silvery sparkle fountain + 10 drifting snowflake particles radiating from the moon, big silver glow halo pulses around the moon for ~12s, continuous ambient sparkle/snowflake emission from the moon for the first 2 seconds, and +5 happiness / +3 care / +5 friendship XP. Special celebration speech ("The snow moon blesses me~! 🌕❄️", "Silver winter blessing~! 🌕✨").
- **Festival lifecycle** — natural 2-minute lifespan; fades in over 180 frames (3s) and fades out over 180 frames (3s). If time turns to morning, season changes away from winter, weather turns rainy/stormy, or the pet goes to sleep while the festival is NOT yet blessed, it gracefully fades out early. Blessed festivals finish playing out their magic undisturbed.
- **Gentle hint text** — faint blinking "catch ❄️" label above the first uncaught snowflake until the player has blessed their first snow moon festival, then disappears forever (matching the hint pattern used by mooncake/fairy/bunny/cardinal/harvest-moon).
- **First-sighting & first-blessing diary milestones** — two separate dated diary entries record the first time a snow moon ever appears ("A silver snow moon rose over my world~! Tiny paper snowflakes drifted in the sky, waiting to be caught! 🌕❄️✨") and the first time all snowflakes are caught ("I caught every snowflake under the snow moon~! The sky answered with a silver winter blessing! 🌕❄️✨"). Subsequent blessings add shorter "Snow moon blessing #N~!" entries.
- **Dream template** — added "snow_moon" to `DREAM_TEMPLATES` so sleeping after a snow moon session yields dreams of "a sky of silver snowflakes~", "moonlit winter festival~", "dancing under the pale-blue moon~".
- **Sleep-talk phrases** — 3 new contextual sleep talks for snow_moon: "*mumble*... silver moon... so pretty...", "Zzz... snowflakes... catching them all...", "*snore*... winter blessing... warm inside...".
- **Contextual dream icons** — `dailyActivityLog` now recognizes "snow_moon" and biases dream icons toward moon/star/heart after interacting with a festival.
- **Snow Moon Blessed achievement** — catch every snowflake under a snow moon to unlock achievement #76 (🌕). Single-completion achievement scaled to match Moon Blessed (harvest moon's equivalent), since snow moons are equally rare.
- **Stats tracking** — total snow moons blessed + total snowflakes caught displayed in the WEATHER section of the stats panel between SNOW BUNNIES and LIGHTNING BOLTS in a cool `#BCD4F0` SNOW MOON row.
- **Full persistence** — totalSnowMoonsSeen, totalSnowflakesCaught, totalSnowMoonsBlessed, snowMoonFirstSeen, and snowMoonFirstBlessed saved across sessions.
- **Total achievements: 76**

**Why this feature:** Last cycle (v0.100 Snow Bunny) filled the last winter *daytime* ground-visitor slot — winter now has a cardinal (sky, daytime), a bunny (ground, daytime), but had **zero** night-time ambience. NEXT.md's *very first* "Thoughts for next cycle" entry opened with: *"Winter night signature event — still completely empty. This is the biggest remaining winter gap (listed repeatedly in prior NEXT.md cycles). Options: snow moon analog to the harvest moon… This feels like the most impactful next step — winter has a daytime cardinal, daytime bunny, but zero night ambience."* That's exactly the gap this cycle fills, and I took the snow-moon-analog-to-harvest-moon option because it creates a clean seasonal-parity pattern the player can learn once and recognize across seasons: autumn-night = harvest moon + lanterns + mooncake reward; winter-night = snow moon + snowflakes + (future) snowflake-keepsake reward. Mechanically the snow moon mirrors the harvest moon's proven multi-stage structure (rise → interact → bless) but inverts the vocabulary so they feel distinct rather than interchangeable reskins: harvest lanterns *hang on strings* and sway in place, snowflakes *drift freely* and rotate; harvest palette is *warm amber/orange* for autumn nostalgia, snow palette is *cool silver/ice-blue* for winter stillness; harvest blessing chime is a *warm pentatonic arpeggio*, snow blessing chime is a *cool A-minor pentatonic with high crystalline shimmer + filtered wind-whisper noise*. The wind-whisper behind the snow blessing is the single most distinctive audio beat — it turns the blessing from "a bell rang" into "the winter sky exhaled" and ties it to the quiet aural texture of real snow. The snow moon also spatially inverts the harvest moon by biasing to the left side of the canvas (vs. harvest's right-bias), so if a player ever sees both across seasons they'll notice the implicit celestial balance. Thematically this completes the four-season night vocabulary: autumn has its harvest moon, winter now has its snow moon, and NEXT.md's future leaps can target spring and summer night events (cherry-blossom moon, summer firework sky) knowing the pattern template is already proven.

## [v0.100.0] — 2026-04-19 — Snow Bunny (Yukiusagi) Visitor

### Added
- **Snow bunny (yukiusagi 雪うさぎ)** — a fluffy white rabbit now occasionally hops into the canvas during winter daytime/evening (any non-stormy weather including snow), hops horizontally across the ground, pauses to sniff the snow with a furiously wiggling pink nose and twitching ears, nibbles at invisible snow-grass 2-4 times with tiny snow puffs, then hops off. Winter's *ground* visitor counterpart to the Winter Cardinal (winter's *sky* visitor), completing the winter small-creature pair the same way autumn has the squirrel + fairy.
- **Hop cycle physics** — bunny advances via real hop cycles: phase increments per frame through `Math.sin(hopPhase)` so the body rises, peaks in the air, lands, and thumps. Each landing plays a soft low-sine thump + high-filtered snow-crunch noise. Horizontal speed is air-boosted — she moves fastest at the peak of each hop, slowest just before touch-down, giving genuinely bunny-like locomotion rather than a flat slide.
- **Paw print trail** — every hop-landing drops a tiny grey paw print (main pad + 3 toe dots, jittered left/right per hop) that fades over ~4.3 seconds. Paw prints persist independently of the bunny so a trail visibly stretches behind her across the ground before the ones at the start melt back into the snow.
- **4-state FSM** — `hopping_in` (enters from left or right edge, hops inward until ~20%/80% into canvas) → `sniffing` (~2.3-4.3s, head-dips 2-3 times with breathy 2-puff "sniff-sniff" sounds, nose wiggles at 0.5 rad/frame) → `nibbling` (~3-5.3s, 2-4 rapid-tick nibbles with snow-puff particles) → `hopping_out` (flips direction and hops off to the nearest screen edge, fades out). Greeting transitions into a special `eating_carrot` state that plays out the carrot-munch sequence before departure.
- **Ear twitch & nose wiggle** — each ear has an independent twitch angle that randomly jumps every 1-3 seconds and decays back to rest at ×0.9/frame, so the two long ears move asymmetrically. Nose wiggles ambient at 0.18 rad/frame, speeds up to 0.5 rad/frame while sniffing/nibbling/eating — the nose literally vibrates when she's engaged.
- **Storybook bunny art** — fluffy white rounded body (~5.6×4.6 px, radial gradient from pure white center → mid → soft shadow rim), fluffy white tail puff behind, tucked-under back leg, two front paws tucked in front, round head with slight forward lean, two long rounded ears (each with **pink inner layer** + inner shadow edge), single tiny black dot eye with white glint, tiny pink wiggling triangle nose, small mouth line, two translucent whiskers radiating forward, squish animation on landing (1.15× horizontal / 0.85× vertical at touch-down, compensated). Sprite mirrors based on `dir: 1 | -1`.
- **3 body-color variants** — 75% classic pure white (most common, yukiusagi-canon), 18% warm cream (a hint of honey-gold), 7% rare pale grey (wild-hare looking). All three share the same pink ear-inner and nose; only the body/shadow palette shifts. Multiple sightings won't always look identical.
- **Click to greet** (~11 px circular hitbox while sniffing/nibbling, non-greeted): plays a sweet 2-tone ascending-then-falling "eeep!" squeak (1800→2600→1600 Hz sine) followed by a nibble burst, and transitions to the `eating_carrot` state — a tiny orange carrot with green leafy top pops into existence near her mouth over 16 frames, then shrinks over ~60 frames as she munches it down to nothing. 12 warm sparkle + 3 heart particles bloom upward, +4 happy / +2 care / +3 friendship XP, contextual greet speech (6 variants like "Carrot for my bunny~! 🥕🐰", "Nibble nibble~! 🐰💕", "Shared my carrot~! 🥕💖").
- **Carrot sprite** — ~4.4×1.8 px cartoon carrot rendered in 2 parts: orange body (linear gradient `#ff9830 → #c05a10`) with 3 thin darker ridges hinting at grooves, and a stylized 2-tone green leafy top (`#3ca048` outer + `#55b060` inner highlight) with 3 leaf points. Shrinks from both X and Y axes as `carrotEaten` rises 0→1.
- **Post-greet hop-off** — after eating the carrot, bunny flips direction toward the nearest screen edge and hops off rather than retracing her path. Nose wiggle slows back down to rest pace on exit.
- **Only one winter visitor at a time** — `canSpawnBunny()` returns false if either a bunny or a cardinal is already on screen, so the two winter creatures never overlap and compete for player attention. Spawn rate `Math.random() < 0.0004` (a touch rarer than the cardinal's 0.0005 to keep each individual visit feeling special).
- **Weather / season / sleep transitions** — if the pet goes to sleep, the season changes away from winter, or the weather turns stormy, the bunny gracefully flips to `hopping_out` and exits. Active `eating_carrot` sequences finish their munch undisturbed since she's earned it.
- **Gentle hint text** — faint blinking "greet 🐰" label above the bunny during sniffing/nibbling states until the player has greeted their first one, then disappears forever (matching cardinal/mooncake/fairy hint pattern).
- **First-sighting & first-greet diary milestones** — two dated diary entries record the first time a bunny ever stops to sniff ("A fluffy snow bunny visited my snow~! Her tiny nose wiggled so fast! 🐰❄️") and the first time one is fed a carrot ("A snow bunny came to my hand~! I shared a carrot and her tiny nose wiggled with joy! 🐰🥕❄️").
- **Dream template** — added "bunny" to `DREAM_TEMPLATES` so sleeping after greeting a bunny yields dreams of "a meadow of bunnies~", "hop hop hop forever~", "soft ears in the snow~".
- **Sleep-talk phrases** — 3 new contextual sleep talks for bunny: "*mumble*... fluffy ears... so soft...", "Zzz... hop hop... carrot yum...", "*snore*... tiny pink nose... wiggle wiggle...".
- **Contextual dream icons** — `dailyActivityLog` now recognizes "bunny" and biases dream icons toward heart + flower + star.
- **Snow Bunny Friend achievement** — greet 3 snow bunnies to unlock achievement #75 (🐰). Scaled to match Cardinal Friend (3 greets) and Fairy Friend (3 greets) as the "small-winter-creature greet" tier.
- **Stats tracking** — total bunnies greeted + total bunnies seen displayed in the WEATHER section of the stats panel in a soft `#F0E0F0` SNOW BUNNIES row between CARDINALS and LIGHTNING BOLTS.
- **Full persistence** — `totalBunniesSeen`, `totalBunniesGreeted`, `bunnyFirstSeen`, and `bunnyFirstGreeted` saved across sessions.
- **Total achievements: 75**

**Why this feature:** Last cycle (v0.99 Winter Cardinal Visitor) filled the "winter has no creature visitors" gap with a sky-based character (cardinal flies diagonally down from the upper sky, perches on the ground, flies off upward). NEXT.md explicitly flagged "Snow bunny (yukiusagi) visitor — second winter character; white fluffy rabbit hops across ground, complements cardinal (one sky, one ground)" as a top follow-up. That's the exact gap this cycle fills, and it maps cleanly onto the existing autumn pattern: autumn has one sky-based small visitor (fairy from fairy ring, hovers in the air) + one ground-based small visitor (squirrel, runs along the ground); winter now has one sky-based (cardinal) + one ground-based (bunny). Mechanically the bunny inverts the cardinal's spatial and motion vocabulary point-for-point: cardinal *arrives from above in a fluttering arc*, bunny *enters low from the side in a hop cycle*; cardinal *perches stationary and chirps*, bunny *hops between pause points and sniffs silently*; cardinal interaction *offers seeds to eat in place*, bunny interaction *offers a carrot that physically appears and shrinks as she munches*. The **carrot-shrink-as-eaten** animation is the first time the game has shown an interactive object visibly consumed in real time (mooncake vanishes after share, seeds were implicit) — a small mechanical first that makes the greet feel more tangible than a particle burst alone. The **paw print trail** is also a new ambient-trace pattern: unlike the cardinal which leaves no trace, the bunny leaves persistent visual history that lingers for seconds after she's moved on, giving the ground state a slight memory of what just happened. Yukiusagi (雪うさぎ — "snow rabbit") are a beloved Japanese winter motif; they appear in folklore, poetry, and even snow-sculpting traditions (children shape small mounds of snow into bunny forms with leaf ears and red-berry eyes), so the feature ties into the same cultural vocabulary as the harvest moon + mooncake additions. Only one of the two winter visitors can be on screen at a time — `canSpawnBunny()` blocks when a cardinal is active and vice-versa — so each visit stays a discrete, attention-worthy moment rather than a cluttered "winter menagerie".

## [v0.99.0] — 2026-04-19 — Winter Cardinal Visitor

### Added
- **Winter cardinal (male northern cardinal)** — a bright red cardinal now occasionally flies into the canvas during winter daytime (morning/afternoon/evening, any non-stormy weather including snow), descends to the ground from above with fluttering wings, perches for 4–7 seconds chirping and pecking for seeds, then flies up and away. Fills the previously empty "winter creature visitor" gap — winter's first ambient character, complementing autumn's squirrel + fairy and spring/summer's butterflies.
- **Flight arc** — enters from the left or right edge at upper-sky y (~20–50 px) with horizontal velocity (1.5 px/frame) and descends in a gentle easing arc toward a target perch x. Wings flap at 0.55 rad/frame while flying (fast) and 0.03 rad/frame while perched (resting). Soft bandpass-filtered noise flutter sounds on wingbeats.
- **Perch behavior** — lands on the ground with feet, does gentle hop-bobs at 0.02 rad/frame, plays an immediate landing "cheer cheer cheer" chirp, then chirps ambiently every 1.2–2.7 seconds. After initial perch, 2-4 pecks at the snow with a small head-dip animation and 3-particle snow-puff per peck.
- **Classic redbird art** — rounded bright-red body (~5.4 × 4.6 px, radial gradient `#ff5060 → #e81828 → #9a0c18`) with darker belly shadow, **prominent pointed crest spike** (2.2 px tall, lifts +1.2 px when alert/greeted), **black face mask** around the eye and at the throat base, **thick orange-yellow conical beak** (linear gradient `#ffc040 → #c0700a` with dividing line), small beady black eye with white glint, layered dark-red tail feathers pointing back-down, two wing layers (back wing behind body, front wing on top flapping at 0.5 rad amplitude when flying), and tiny dark feet with spread toes when perched. Sprite mirrors based on facing direction (`dir: 1 | -1`).
- **"Cheer cheer cheer" chirp** — signature descending 3-tone sine call at ~3600 → 3200 → 2900 Hz with quick 0.04s rising slides + 0.14s falling slides, 0.12s between tones, ~0.17s per note; unmistakably "cardinal" audio.
- **Click to greet** — generous ~10 px circular hitbox; clicking a perched (non-flying, non-greeted) cardinal offers seeds: bright musical trill "what-cheer what-cheer birdie-birdie-birdie" (8-note ascending-then-wavering pattern 2600 → 3800 Hz), 14 warm red sparkle particles + 3 heart particles floating up, crest raises to full height + a small bow-lean animation (2 px forward tilt over ~40 frames easing back), +4 happiness / +2 care / +3 friendship XP, and a contextual greet speech (6 variants like "Shared seeds with the cardinal~! 🐦💖", "A song for my seeds~! 🐦🎵").
- **Greet extends perch** — on greet the cardinal pauses any pecking, stays perched longer with a small victory chirp, and can no longer be greeted again this visit (prevents spam).
- **Natural departure** — after the perch timer expires the cardinal flips direction, flaps hard upward (vy -1.1 accelerating at -0.03/frame, horizontal 1.5 px/frame), plays a flutter sound, and exits the top or side of the canvas before fading out (45 frames).
- **Weather / season / sleep transitions** — if the pet goes to sleep, the season changes away from winter, or the weather turns stormy, the cardinal flips immediately to the flying_out state with a flutter sound and exits gracefully.
- **Gentle hint text** — faint blinking "greet 🐦" label above the perched cardinal until the player has greeted their first one, then disappears forever.
- **First-sighting & first-greet diary milestones** — two separate dated diary entries record the first time a cardinal ever perches ("A bright red cardinal visited my snow~! His feathers glowed against the frost!") and the first time one is greeted ("A red cardinal came to me~! I shared some seeds and he sang his winter song!").
- **Dream template** — added "cardinal" to DREAM_TEMPLATES so sleeping after greeting a cardinal yields dreams of "redbird in the snow~", "cheer-cheer-cheer forever~", "a winter songbird friend~".
- **Sleep-talk phrases** — 3 new contextual sleep talks for cardinal: "*mumble*... cheer cheer... red feathers...", "Zzz... bright redbird... snowy day...", "*snore*... tiny seeds... for my friend...".
- **Contextual dream icons** — dailyActivityLog now recognizes "cardinal" and biases dream icons toward heart + star + flower. Also added the previously missing "mooncake" entry to `getContextualDreamIcons()` while I was in there.
- **Cardinal Friend achievement** — greet 3 winter cardinals to unlock achievement #74 (🐦). Scaled to match the "greet a small visiting creature" achievements like Fairy Friend (3 fairies).
- **Stats tracking** — total cardinals greeted + total cardinals seen displayed in the WEATHER section of the stats panel in a bright `#E82838` CARDINALS row between MOONCAKES and LIGHTNING BOLTS.
- **Full persistence** — totalCardinalsSeen, totalCardinalsGreeted, cardinalFirstSeen, and cardinalFirstGreeted saved across sessions.
- **Total achievements: 74**

**Why this feature:** Last cycle (v0.98 Mooncake Reward Drop) completed the autumn-night narrative arc. NEXT.md explicitly flagged "Winter has NO creature visitors currently — big gap" and listed winter cardinal as a top candidate: "a bright red bird that lands on the ground or a tree branch during snowy/winter weather, nibbles at seeds, chirps, and flies away. Iconic winter creature." That's the exact gap this cycle fills. Cardinals are the iconic North American winter bird — their vivid red against snow is one of the most recognizable winter images in nature, and they're among the few songbirds that stay active through the coldest months. Mechanically the cardinal mirrors the squirrel's visit arc (arrive → pause → do a small action → leave) but inverts the spatial and interaction vocabulary: the squirrel enters low and horizontal along the ground, drops acorns on the ground that the player must collect; the cardinal enters *high and diagonal from the sky*, and the player interacts with the bird *itself* rather than with dropped objects. The chirp-first / click-to-greet pattern also mirrors the fairy's interaction (a brief character encounter with a greeting reward) but grounded on the ground rather than hovering in the air. Together these three small-creature visits (squirrel in autumn, fairy after fairy rings, cardinal in winter) now form a seasonal triptych of "small living friends" interactions. The "cheer-cheer-cheer" descending chirp is the most distinctive vocal signature of real cardinals, so nailing that audio was the single most important identity beat for the feature — a player should hear it and immediately know "that's a cardinal" without needing to look. Winter still lacks a big signature-event like harvest moon, but now it at least has a recurring friendly visitor to break up the stillness of snowy days, making the ~25% of gameplay spent in winter feel much more alive.

## [v0.98.0] — 2026-04-18 — Mooncake Reward Drop

### Added
- **Mooncake reward drop** — after every Harvest Moon Blessing, the moon now drops a traditional Chinese mooncake (月餅) as a harvest-night gift. The mooncake spawns at the moon's current position and arcs gracefully down to the ground with a gentle parabolic fall, a tiny hump over its path, rotational spin during the drop, and a soft dust puff on landing.
- **Descending chime** — a soft descending twinkle-glissando (G6→E6→C6→A5 sine tones, ~0.35s each) plays as the mooncake drops, announcing the gift audibly and tying the sound back to the harvest moon blessing's pentatonic vocabulary.
- **Landing thump** — when it hits the ground, a warm low sine thump (160→90Hz, ~0.22s) plays with a 6-particle dust puff and a tiny bounce-in squash animation (1.3× horizontal / 0.75× vertical, eases back over ~25 frames).
- **Traditional mooncake sprite** — cartoon ~14×8 px round mooncake rendered in three layers: golden-brown pastry *body* ellipse (radial `#E6B566 → #8C5A28`), lighter golden *top surface* ellipse (radial `#F4D090 → #A57438`), and a decorative **4-petal lotus imprint** traced as four quadratic Bézier curves from the center outward. Center has a stamped amber-yolk dot (`#E8A850` outer + `#A56020` inner), 8 cream-colored fluted-edge rim dots around the crust, and an upper-left specular highlight.
- **Pulsing warm halo** — while the mooncake sits on the ground waiting to be picked up, a soft warm amber halo (`rgba(255, 200, 110, 0.4)` → transparent) pulses at `sin(glowPulse)` around it at 10–13 px radius, inviting the player to notice and click.
- **Gentle "share 🥮" hint** — a faint blinking label floats above the mooncake until the player has shared their first one, then disappears forever (matching the hint pattern used by other interactive elements).
- **Click to share** — generous ~9 px circular hitbox; clicking the mooncake shares it with the pet: ascending warm E5-G5-C6 triangle arpeggio + a low A3 "mmm" body sustain, 14-particle sparkle burst + 3 golden heart particles floating up, +6 happiness / +20 hunger / +2 care points / +4 friendship XP, and a contextual share speech ("Mmm~ mooncake! 🥮✨", "A harvest feast just for me~! 🥮🌕", "Sharing with the moon~! 🥮✨").
- **Natural lifespan** — mooncake sits on the ground for ~40 seconds before fading out over 60 frames if uncollected, so the ground stays tidy. Fades out faster if the pet goes to sleep while a mooncake is still on the ground.
- **30% silent sighting / 70% chatty sighting** — 70% of mooncake spawns trigger a contextual sighting speech ("A mooncake~! 🥮", "Whooa~ a gift from the moon! 🥮🌕") so you don't hear the same line every time.
- **First-share diary milestone** — a dated diary entry records the first time a mooncake is shared ("The moon gave me a mooncake after the harvest blessing~!"). Subsequent shares add shorter "Shared mooncake #N~!" entries.
- **Mooncake Keeper achievement** — share a mooncake from a blessed harvest moon to unlock achievement #73 (🥮). Single-completion achievement, scaled to match how rare harvest moons already are.
- **Dream template** — "mooncake" added to DREAM_TEMPLATES so sleeping after sharing a mooncake yields dreams of "a mountain of mooncakes~", "a lotus-golden feast~", "sharing sweets with the moon~".
- **Sleep-talk phrases** — 3 new contextual sleep talks: "*mumble*... one more bite... lotus yum...", "Zzz... moon gave me cake... hehe...", "*snore*... golden mooncake... sticky sweet...".
- **Contextual dream icons** — dailyActivityLog now recognizes "mooncake" (and finally "harvest_moon" which was missing from the biasing list) and biases dream icons toward food/moon/heart after a mooncake is shared.
- **Stats tracking** — total mooncakes shared + total mooncakes dropped displayed in the WEATHER section of the stats panel in a warm `#E8B880` MOONCAKES row between HARVEST MOON and LIGHTNING BOLTS.
- **Full persistence** — totalMooncakesShared, totalMooncakesDropped, and mooncakeFirstShared saved across sessions.
- **Total achievements: 73**

**Why this feature:** Last cycle (v0.97 Harvest Moon Festival) introduced the blessing event — light every lantern under a giant golden moon for a big celebration chime and sparkle fountain. NEXT.md explicitly listed "Mooncake reward drop — rare chance that after a harvest moon blessing, a small mooncake appears on the ground for the pet to walk to and nibble (small stamina bonus + unique speech). Listed alongside harvest moon originally; this cycle only delivered the core moon + lanterns" as the direct natural followup. That's exactly what this cycle delivers: a tangible tactile reward object that *drops from the moon* as the final payoff beat of the blessing. Mechanically this is the first time the game has used a **scripted multi-stage payoff**: blessing → sparkle fountain → glow halo → moon drops a gift → gift falls to ground → player shares it → nibble celebration. Previously rewards were either instant (XP/stats/particles) or long-lived ambient (e.g., fairy dance after ring completion). The mooncake sits in between — it's a physical collectible that's earned rather than random, and tying it to the already-rare harvest moon event keeps it a precious moment rather than a repeatable grind. Thematically, mooncakes are the defining food of the Mid-Autumn Festival (中秋節), where they're shared with family under the harvest moon — so giving the pet a mooncake as the moon's gift completes the autumn-night narrative arc with a genuinely culturally-resonant object. The descending twinkle chime + warm share arpeggio extend the existing "harvest moon audio vocabulary" (pentatonic blessing arpeggio, temple-bell lantern-lit chime) into a coherent musical language that the player now associates with the entire harvest moon festival experience.

## [v0.97.0] — 2026-04-18 — Harvest Moon Festival

### Added
- **Harvest Moon Festival** — a rare autumn-night special event where an oversized golden-orange harvest moon rises into the sky alongside 3-5 paper lanterns hanging in the air, waiting to be lit. The festival lasts ~2 minutes and can only appear during autumn nights in clear weather.
- **Oversized golden moon** — significantly larger than typical night-sky elements (radius ~22-26 px on a 200×200 canvas), rendered with a radial gradient from warm amber `#FFE8A0` center to deep autumn orange `#A45820` rim, subtle crater details (three darker spots), upper-left specular highlight, and a breathing outer warm-glow halo that pulses at `sin(frame * 0.02)`.
- **Moon rise animation** — when the festival spawns, the moon starts 50 px below its resting position and eases upward with a cubic ease-out over 10 seconds, so you actually watch it rise into the sky.
- **3-5 paper lanterns** — cartoon Japanese-style chochin lanterns (提灯) hanging from invisible strings running up to the top of the canvas. Each lantern has a dark wooden top cap, rounded oval paper body with vertical ribs, top/bottom dark bands, and a gold tassel with knot dangling below. Unlit lanterns are muted brown; lit lanterns bloom with colored paper, inner bright core glow, and outer halo.
- **4 lantern color variants** — randomized per lantern: red (hue 8), orange (hue 24), gold (hue 42), warm cream (hue 52). Hue/sat/light values drive the paper color, the halo color, the rib color, and the inner bright core — each lantern feels like its own distinct paper character.
- **Gentle sway animation** — each lantern swings with an independent sine-wave sway (phase offset, speed variance 0.020–0.035 rad/frame) so the whole set has a hand-hung feel, not a uniform mechanical wobble.
- **Click to light** — click an unlit lantern (elliptical hitbox `7 × 10 px`) to light it: soft two-note A5+E6 triangle-wave temple-bell chime + warm A4 fundamental for body, 8-particle sparkle bloom, lantern paper turns from muted brown to full-color glow over ~25 frames (`litAnim 0→1` at +0.04/frame), +1 happiness / +1 friendship XP, 50% chance of speech ("Another one~! 🏮", "Keep going, light them all~! 🏮").
- **Lit-lantern ambient sparkles** — once lit, each lantern emits a tiny warm sparkle particle below its body every 40-70 frames, so the lit sky has a gentle shimmer life of its own.
- **Harvest Moon Blessing** — lighting *every* lantern triggers the big festival payoff: golden pentatonic arpeggio (C5-E5-G5-C6-E6-G6) + a G7 shimmer chord on top, 28-particle sparkle fountain radiating from the moon, big golden glow halo pulses around the moon for ~12s, continuous ambient sparkle emission from the moon for the first 2 seconds, and +5 happiness / +3 care / +5 friendship XP. Special celebration speech ("The harvest moon blesses me~! 🌕✨", "Moonlight festival~! 🌕🏮💖").
- **Festival lifecycle** — natural 2-minute lifespan; fades in over 180 frames (3s) and fades out over 180 frames (3s). If time turns to morning, season changes away from autumn, weather turns to rain/storm/snow, or the pet goes to sleep while the festival is NOT yet blessed, it gracefully fades out early. Blessed festivals finish playing out their magic undisturbed.
- **Gentle hint text** — faint blinking "light 🏮" label above the first unlit lantern until the player has blessed their first harvest moon festival, then disappears forever.
- **First-sighting & first-blessing diary milestones** — two separate dated diary entries record the first time a harvest moon ever appears ("A giant harvest moon rose over my world~!") and the first time all lanterns are lit ("I lit every lantern under the harvest moon~!"). Subsequent blessings add shorter "Harvest moon blessing #N~!" entries.
- **Dream template** — added "harvest_moon" to DREAM_TEMPLATES so sleeping after a harvest moon session yields dreams of "a sky of golden lanterns~", "moonlit harvest festival~", "dancing under the big orange moon~".
- **Sleep-talk phrases** — 3 new contextual sleep talks: "*mumble*... big golden moon... so pretty...", "Zzz... lanterns floating... lighting... up...", "*snore*... harvest blessing... warm glow...".
- **Contextual dream icons** — dailyActivityLog now recognizes "harvest_moon" and biases dream icons toward moon/star/heart when the pet sleeps after interacting with a festival.
- **Moon Blessed achievement** — light every lantern under a harvest moon to unlock achievement #72 (🌕). Single-completion achievement since harvest moons are rare events.
- **Stats tracking** — total harvest moons blessed + total lanterns lit displayed in the WEATHER section of the stats panel between FAIRIES and LIGHTNING BOLTS in an amber `#F2B060` HARVEST MOON row.
- **Full persistence** — totalHarvestMoonsSeen, totalHarvestLanternsLit, totalHarvestMoonsBlessed, harvestMoonFirstSeen, and harvestMoonFirstBlessed saved across sessions.
- **Total achievements: 72**

**Why this feature:** Autumn now has four daytime/evening interactions (leaf pile, squirrel + acorns, mushroom ring, fairy) — but *autumn nights* were completely empty. No signature event, no unique visual, no reason to keep Tamashii open past sunset during October. The Harvest Moon Festival fills that gap with the kind of payoff the other autumn features deserve: an oversized golden moon (the visual centerpiece), 3-5 floating lanterns (the interactive layer), and a "light them all" completion puzzle with a big golden blessing payoff (the emotional beat). It mirrors the multi-step completion pattern of the mushroom ring — each lantern is a small click-for-reward action, but lighting *all* of them triggers a bigger celebration — while inverting the scale: mushroom rings are tiny ground-level circles, harvest moons fill the sky. NEXT.md listed "Harvest moon festival — special autumn-night event with oversized orange moon, lantern glows, unique mooncake item" as a candidate, and this cycle delivers the core moon + lanterns arc (without the mooncake item — that can be layered on a future cycle as a reward drop). The feature also introduces the game's first *pure-sky* interactive element: unlike the fairy (body-level) or the mushroom ring (ground-level), lanterns hang in the upper canvas and invite the player to look *up*. That vertical framing — eyes raised to the moon, cursor tracing paths between glowing paper lights — extends the game's spatial vocabulary in a way no prior feature has done.
